...
# Optimal Controller Presets (not currently used, since we use the cartesian controller instead)
WEIGHT = 1.0
...
if __name__ == '__main__':
  ...
  rainbow_interface = RainbowInterface(address='192.168.30.1:50051',
                                       power_device='.*',
                                       servo='.*')
  
  controller_type = 'cartesian'
  speed_reduction_factor = None
  while speed_reduction_factor is None:
    try:
      speed_reduction_factor = float(input(' >> Enter desired speed reduction factor: '))
    except ValueError:
      pass
  
  # Load the trajectories to use.
  print('Loading the trajectories')
  T_right = np.load('T_right.npy')
  T_left = np.load('T_left.npy')
  T_torso = np.load('T_torso.npy')
  t = np.linspace(0, 10, num=100)
  
  # Move to the starting pose.
  print('Moving to the starting pose')
  rainbow_interface.move_to_pose(
      T_right=T_right[0],
      T_left=T_left[0],
      T_torso=T_torso[0],
  ...
  (
    ...
    T_right_interp,
    T_torso_interp,
  ) = rainbow_interface.run_trajectory_and_record(
      t,
      T_left=T_left,
      T_right=T_right,
      T_torso=T_torso,
  ...
  # Plot how well the trajectory was tracked.
  print('Saving plots of the trajectory tracking...')
